...
use cosmwasm_std::{
    from_slice, to_binary, Binary, DepsMut, IbcChannel, IbcOrder, IbcPacket,
...
pub fn on_recv_packet(
    ...
    let ack_data = match msg.query {
        ...
        GammMsg::EstimateSwap(m) => {
            let re: QueryRequest<OsmosisQuery> = QueryRequest::Custom(
                OsmosisQuery::estimate_price(m.sender, m.pool.u64(), m.token_in, m.token_out, SwapAmount::In(m.amount))
            );
